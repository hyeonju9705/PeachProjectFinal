    ...
    <div>
      <table class="table table-striped table-bordered table-hover">
		...
		<div>
			<c:if test="${page.prev}">
 				<span>[ 
 				<a href="/community/listPage?num=${page.startPageNum - 1}">이전</a> 
 				]</span>
			</c:if>
			...
			<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">  				
  				<span>
					...
					<c:if test="${select != num}">
   						<a href="/community/listPage?num=${num}">${num}</a>
  					</c:if>    
			...
			<c:if test="${page.next}">
 				<span>[ <a href="/community/listPage?num=${page.endPageNum + 1}">다음</a> ]</span>
			</c:if>
		</div>
</body>
